...
CHART_DIR="charts/koreader-sync-server"
...
echo "Next chart version from commits: ${new_version}"
...
prettier --write README.md "${CHART_DIR}/README.md" CHANGELOG.md

echo "Updating CHANGELOG.md with git-cliff..."
git cliff --unreleased --tag "${tag}" --prepend CHANGELOG.md

git add "${CHART_FILE}" CHANGELOG.md "${CHART_DIR}/README.md"
